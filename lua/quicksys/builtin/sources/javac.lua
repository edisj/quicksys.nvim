...
return {
  ...
  handler = function(data)
    ...
    local error_pattern = "^([^:]+):(%d+): ([^:]+): (.+)$"
    local line_to_qf_item = function(line)
      local filename, lnum, severity, message = line:match(error_pattern)
  ...
  qftf = function(info)
    ...
    for _, item in ipairs(items) do
      local full_name = vim.api.nvim_buf_get_name(item.bufnr)
      local rel_name = vim.fn.fnamemodify(full_name, ":.")
      local pad = string.rep(" ", lnum_max - #tostring(item.lnum))
      local formatted_line = (filename_fmt..":%d: "..pad.."%s: %s"):format(
        rel_name,
        item.lnum,
